...
export interface GeminiEnhancedRiskAssessmentResult extends RiskAssessmentResult {
...
export interface GeminiVectorEnhancedRiskInput {
  patient: Patient;
...
export class GeminiEnhancedRiskAssessmentEngine extends RiskAssessmentEngine {
  private geminiService: SimpleGeminiService;

  constructor(geminiService?: SimpleGeminiService) {
    super();
    this.geminiService = geminiService || new SimpleGeminiService();
  }
  ...
  async initialize(): Promise<void> {
    try {
      ...
      console.log('✅ Gemini Enhanced Risk Assessment Engine initialized');
    } catch (error) {
      console.error('❌ Failed to initialize Gemini Enhanced Risk Assessment Engine:', error);
      throw error;
    }
  }
   ...
   */
  async assessPatientRiskEnhanced(input: GeminiVectorEnhancedRiskInput): Promise<GeminiEnhancedRiskAssessmentResult> {
    try {
      // Get traditional risk assessment
      const baseAssessment = super.assessPatientRisk(input.patient);
      
      // Initialize enhanced result
      const enhancedResult: GeminiEnhancedRiskAssessmentResult = {
        ...baseAssessment,
        relevantGuidelines: [],
        aiConfidence: 0,
  ...
  async searchRelevantGuidelines(
    ...
    try {
      ...
      const results = await this.geminiService.searchDocuments(searchQuery);
  ...
  async getAIExplanation(
    ...
    try {
      ...
      const ragContext = await this.geminiService.createRAGContext(patient, query);
      
      return await this.geminiService.generateAIExplanation(ragContext);
    } catch (error) {
    ...
    try {
      const documents = await Promise.all(
        guidelines.map(async (guideline) => {
          const chunks = await this.geminiService.processDocument(guideline.content, {
            id: guideline.title,
      ...
      await this.geminiService.addDocuments(allChunks);
  ...
  async getVectorDBStats() {
    try {
      return await this.geminiService.getStats();
    } catch (error) {
  ...
  private async enhanceWithVectorSearch(
    result: GeminiEnhancedRiskAssessmentResult, 
    input: GeminiVectorEnhancedRiskInput
  ): Promise<void> {
    try {
      const query = input.customQuery || this.generateContextualQuery(input.patient);
      const ragContext = await this.geminiService.createRAGContext(input.patient, query);
  ...
  private async enhanceWithAIExplanation(result: GeminiEnhancedRiskAssessmentResult): Promise<void> {
    try {
      if (!result.ragContext) {
        throw new Error('RAG context is required for AI explanation');
      }

      const aiExplanation = await this.geminiService.generateAIExplanation(result.ragContext);
